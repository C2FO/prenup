...
- Subcommands: `run`, `plan`, `install`, `uninstall`, `init`, `migrate`,
...
- `prenup migrate` to convert v1 `.prenup.yml` files to the current v2
  schema.
- Embedded JSON Schema for editor `$schema` integration, published at
  `assets/prenup.schema.json`.
